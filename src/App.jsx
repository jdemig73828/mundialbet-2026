...
const getSupabaseClient = async () => {
  ...
  try {
    if (window.supabase) {
      supabaseClientInstance = window.supabase.createClient(url, key);
...
const getLocalDb = () => {
  const defaultDb = {
    ...
    picks: [],    
    matches: [
      { id: 1, phase: '3º y 4º Puesto', date: '18 Jul 2026', team1: 'Por definir', flag1: '❓', score1: '-', team2: 'Por definir', flag2: '❓', score2: '-', status: 'Próximamente' },
      { id: 2, phase: '🏆 Gran Final', date: '19 Jul 2026', team1: 'Por definir', flag1: '❓', score1: '-', team2: 'Por definir', flag2: '❓', score2: '-', status: 'Próximamente' }
    ]
  };
...
export default function App() {
  ...
  const fetchUserData = async (client, userId) => {
    try {
      ...
      const { data: profData } = await client.from('profiles').select('*').eq('id', userId).single();
      if (profData) setProfile({ name: profData.name || '', phone: profData.phone || '', has_paid: profData.has_paid, points: profData.points || 0 });

      const { data: picksData } = await client.from('picks').select('*').eq('user_id', userId).single();
      if (picksData) {
    ...
    try {
      if (!client) {
        ...
        const merged = db.profiles.map(prof => {
          const pk = db.picks.find(p => p.user_id === prof.id);
          return {
            id: prof.id, name: prof.name || 'Participante', phone: prof.phone || '', points: prof.points || 0, hasPaid: prof.has_paid,
            picks: pk ? { first: pk.first_place, second: pk.second_place, third: pk.third_place, fourth: pk.fourth_place } : null
          };
      ...
      const { data: picks } = await client.from('picks').select('*');
      const merged = (profiles || []).map(prof => {
        const pk = (picks || []).find(p => p.user_id === prof.id);
        return {
          id: prof.id, name: prof.name || 'Participante', phone: prof.phone || '', points: prof.points || 0, hasPaid: prof.has_paid,
          picks: pk ? { first: pk.first_place, second: pk.second_place, third: pk.third_place, fourth: pk.fourth_place } : null
        };
      ...
      const { data: matches } = await client.from('matches').select('*').order('id', { ascending: true });
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      if (!client) {
        ...
        if (isRegistering) {
          if (db.profiles.some(p => p.id === id)) throw new Error("El usuario ya existe.");
          db.profiles.push({ id, name: loginEmail.split('@')[0], phone: '', has_paid: false, points: 0 });
          saveLocalDb(db);
          showToast("Registro completado (Modo Local).", 'success');
        } else {
      ...
      if (isRegistering) {
        const { data, error } = await client.auth.signUp({ email: loginEmail, password: loginPass });
        if (error) throw error;
        if (data.user) {
          await client.from('profiles').insert({ id: data.user.id, name: loginEmail.split('@')[0], phone: '', has_paid: false, points: 0 });
          setUser({ id: data.user.id, email: data.user.email, name: loginEmail.split('@')[0] });
          await fetchUserData(client, data.user.id);
          showToast("Registro completado. Sesión iniciada.", 'success');
          setIsRegistering(false);
        }
      } else {
        const { data, error } = await client.auth.signInWithPassword({ email: loginEmail, password: loginPass });
        if (error) throw error;
        if (data.user) {
    ...
    try {
      ...
      if (!client) {
        ...
        if (profIdx > -1) {
          ...
          db.profiles.push({ id: user.id, name: profile.name, phone: profile.phone, has_paid: false, points: 0 });
        }
        db.picks = db.picks.filter(p => p.user_id !== user.id);
        db.picks.push({ id: 'p_user_' + user.id, user_id: user.id, first_place: userPicks.first, second_place: userPicks.second, third_place: userPicks.third, fourth_place: userPicks.fourth });
        saveLocalDb(db);
        setIsLocked(true);
        showToast("Pronósticos asegurados.", 'success');
        await loadGlobalData(null);
        setSubmitting(false);
        return;
      }

      await client.from('profiles').upsert({ id: user.id, name: profile.name, phone: profile.phone, has_paid: profile.has_paid, points: profile.points });
      await client.from('picks').insert({ user_id: user.id, first_place: userPicks.first, second_place: userPicks.second, third_place: userPicks.third, fourth_place: userPicks.fourth });
      
      setIsLocked(true);
      showToast("Apuesta guardada con éxito.", 'success');
      await loadGlobalData(client);
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center text-slate-300">
        ...
        <p className="font-semibold text-slate-400 tracking-wide">Iniciando Portal...</p>
      </div>
    );
  }
    ...
    <div className="font-sans bg-slate-950 min-h-screen text-slate-300 select-none">
      ...
      {currentTab === 'dashboard' && (
        <Dashboard 
          ...
          userPicks={userPicks} updatePick={updatePick} isLocked={isLocked}
          leaderboardData={leaderboardData} matchesData={matchesData}
          submitting={submitting} requestLockPicks={requestLockPicks}
        />
      )}
